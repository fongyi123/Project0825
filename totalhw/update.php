...
<?	/*
	...
	echo "</pre>";
	*/
	$sql = "select * from person where number = ".$_POST["number"];
...
//		echo dirname(__FILE__)."/";							//自動抓取上層前目錄
		$server_path = dirname(__FILE__)."/images";
		unlink($server_path.'/'.$row['Image']);							//刪除
		...
		move_uploaded_file($_FILES["file"]["tmp_name"],$server_path."/".$file);
	}
...
//		echo $special;
	...
	if(empty($_POST["id"]) or empty($_POST["name"]) or empty($_POST["sex"]) or empty($_POST["id_card"]) or empty($_POST["office"]) or empty($_POST["birthday"])){
		...
		/*
		$sql = "update person set Id = '".$id."',Name = '".$name."',EName = '".$ename."',Sex = '".$sex."',Id_Card = '".$id_card."',Office = '".$office."',Country = '".$country."',Birthday = '".$birthday."',Telephone = '".$telephone."', Cellphone = '".$cellphone."',Email = '".$email."',Letter_city = '".$letter_city."',Letter_towns = '".$letter_towns."',Letter_postal = '".$letter_postal."',Letter = '".$letter."',Address_city = '".$address_city."',Address_towns = '".$address_towns."',Address_postal = '".$address_postal."',Address = '".$address."',Person = '".$person."',Connection = '".$connection."',Wedding = '".$wedding."',Soldier = '".$soldier."', Special = '".$special."',Driver1 = '".$driver1."',Driver2 = '".$driver2."',Driver3 = '".$driver3."',Driver4 = '".$driver4."',Image = '".$file."' where number = ".$_POST['number'];
		...
		exit;
		*/
		$prepare = $db -> execute("select * from person where number = ".$_POST["number"]);
		$updatetable = array(
			'Id' => $id,
			'Name' => $name,
			'EName' => $ename,
		...
		echo $sql = $db -> getUpdateSql($prepare,$updatetable);
...
<form action="exam.php" id="form" name="form" method="post">
<table width="100%" border="0" align="center">
  ...
  <tr align="center">
    <td><input name="back" id="back" type="submit" value="取消" /></td>
  </tr>
</table>
</form>
