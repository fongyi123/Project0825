...
class MY_Pagination {
	...
	public function render() {
		...
		if ($num_pages > 1) {
			...
			for ($i = $start; $i <= $end; $i++) {
				if ($page == $i) {
					$output .= ' <li class="active"><a href="#"><b>' . $i . '</b></a></li> ';
				} else {
		...
		return ($output ? '<div class="' . $this->style_links . '"><ul class="pagination">' . $output . '</div>' : '') . '<div class="' . $this->style_results . '">' . str_replace($find, $replace, $this->text) . '</ul></div>';
	}
}